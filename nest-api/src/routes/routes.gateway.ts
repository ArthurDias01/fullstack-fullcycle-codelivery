...
export class RoutesGateway implements OnModuleInit {
  ...
  sendPosition(data: { routeId: string, clientId: string, position: [number, number], finished: boolean }) {
    const { clientId, ...rest } = data;

    const clients = this.server.sockets.connected;

    if (!(clientId in clients)) {
      console.error('Client not found, refresh REACT APP and resend new direction.');
      return;
    }
